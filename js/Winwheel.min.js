...
/* ============================================================
   Weighted prize helpers (added)
   - Allows controlling hit-rate per segment by setting stopAngle
   - Works with animation.type = 'spinToStop' (recommended)
   ============================================================ */

function winwheelPickWeightedIndex(weights) {
    // weights: Array of non-negative numbers, weights[0] is segment 1
    ...
    for (i = 0; i < weights.length; i++) {
        var wi = Number(weights[i]);
        if (!isFinite(wi) || wi < 0) wi = 0;
        r -= wi;
        if (r <= 0) return i + 1; // Winwheel segments are 1-based
    }
    return weights.length;
}

Winwheel.prototype.setStopAngleForSegment = function(segIndex) {
    // Sets animation.stopAngle to a random safe angle inside segment segIndex.
    // Returns the chosen stopAngle (degrees).
    if (!this.animation) this.animation = new Animation;
    var ang = this.getRandomForSegment(segIndex);
    this.animation.stopAngle = ang;
    return ang;
};

Winwheel.prototype.setPrizeByWeights = function(weights) {
    // Picks a segment index by weights and sets stopAngle accordingly.
    // Returns picked segment index (1-based).
    var idx = winwheelPickWeightedIndex(weights);
    // Clamp to available segments.
    if (idx < 1) idx = 1;
    if (idx > this.numSegments) idx = this.numSegments;
    this.setStopAngleForSegment(idx);
    return idx;
};

Winwheel.prototype.setPrizeByText = function(text) {
    // Finds first segment whose .text matches exactly and sets stopAngle.
    // Returns segment index, or 0 if not found.
    if (!this.segments) return 0;
    for (var i = 1; i <= this.numSegments; i++) {
        if (this.segments[i] && this.segments[i].text === text) {
            this.setStopAngleForSegment(i);
            return i;
        }
    }
    return 0;
